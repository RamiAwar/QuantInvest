...
class GetDataTests(unittest.TestCase):
    ...
    def test_get_all_snp500_data(self):
        current_date = datetime.now()
        df = get_all_snp500_data(current_date-relativedelta(years=1), current_date)
    ...
    def test_get_data_uncached_stocks(self):
        current_date = datetime.now()
        df = get_data(['AA','AAPL'], current_date-relativedelta(month=1), current_date)
        print(df)
        self.assertTrue(True)
